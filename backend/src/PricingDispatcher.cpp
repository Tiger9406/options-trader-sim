...
std::vector<double> PricingDispatcher::priceBatch(const OptionBatch& batch, int steps) {
    ...
    #pragma omp for
        for (int i = 0; i < N; ++i) {
            Option opt{
                    batch.S[i], batch.K[i], batch.r[i], batch.sigma[i], batch.T[i],
                    batch.type[i], batch.style[i]
            };

            if (opt.style == OptionStyle::European) {
                prices[i] = BlackScholes::price(opt);
            } else {
                prices[i] = BinomialTree::price(opt, steps, workspace);
            }
        }
    }
